...
def aStarSearchGreedy(problem: SearchProblem, heuristic=nullHeuristic):
    "*** YOUR CODE HERE ***"
    pathG = Path([problem.getStartState()], [], 0)

    if problem.isGoalState(problem.getStartState()):
        return pathG.directions
        ...
        if problem.isGoalState(currentLocation):
            ...
            for nextStep in nextSteps:
                ...
                if (nextLocation not in currentPath.locations) and (nextLocation not in visited):
                    ...
                    nextPath = GreedyPath(nextLocations, nextDirections)

                    queue.push(nextPath, nextHeuristic)
    return []
