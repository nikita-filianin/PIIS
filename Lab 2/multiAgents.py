...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def avgValue(self, gameState, currentDepth, agentIndex):
        ...
        for action in gameState.getLegalActions(agentIndex):
            if agentIndex == gameState.getNumAgents() - 1:
                avgValue = avgValue + self.getValue(gameState.generateSuccessor(agentIndex, action), currentDepth + 1,
                                                    0)
            else:
                avgValue = avgValue + self.getValue(gameState.generateSuccessor(agentIndex, action), currentDepth,
                                                    agentIndex + 1)
        return avgValue
